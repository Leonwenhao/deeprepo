"""Manual RLM integration script for tests/test_small/.

This file intentionally contains no pytest tests and does not execute on import.
Run it directly for a live end-to-end integration/cost quality check.
"""
...
def main() -> None:
    ...
    checks = [
        (
            ...
            any(
                x in analysis_lower
                for x in ["sql injection", "sql-injection", "f-string", "f\"select", "string format"]
            ),
        ),
        (
            "Hardcoded secret key",
            any(x in analysis_lower for x in ["secret_key", "hardcoded", "hard-coded", "secret key"]),
        ),
        (
            "Debug mode",
            any(x in analysis_lower for x in ["debug=true", "debug mode", "debug"]),
        ),
        (
            "MD5 hashing",
            any(x in analysis_lower for x in ["md5", "hash", "weak hash", "insecure hash"]),
        ),
        (
            "Unclosed DB connections",
            any(x in analysis_lower for x in ["connection", "close", "unclosed", "not closed", "resource leak"]),
        ),
    ]
    ...
    if cost < 0.50:
        ...
        print("  WARNING: Cost exceeded $0.50 target")
    ...
    if found >= 3 and len(analysis) > 500:
        ...
        print(
            f"Integration test PARTIAL — detected {found}/5 bugs, "
